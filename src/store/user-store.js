...
class UserStore {
  ...
  constructor() {
    ...
    reaction(
      () => this.favoritProducts.length > 0 && this.favoritProducts.slice(), // Використовуйте .slice() для створення копії масиву, якщо необхідно
      (favoritProducts) => {
    ...
    reaction(
      () => this.adresses.length > 0 && this.adresses.slice(),
      (adresses) => {
        if (this.isAuthenticated) {
