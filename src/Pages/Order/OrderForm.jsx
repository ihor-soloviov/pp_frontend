...
import {setOrderData, setPaymentData, setPosterResponsea} from "../../store/orderSlice";
...
const OrderForm = () => {
        ...
        const url = 'https://polarpelmeni-api.work-set.eu/api'
        const headers = {
        ...
        /*
        https://polarpelmeni-api.work-set.eu/api/pay
        https://polarpelmeni-api.work-set.eu/api/promocode
        */
    ...
    Кол-во персон: ${formData.personCount},
      ${formData.comment && ` ${formData.comment}`}, ${
        ...
        const usagePromotion = () => {
            ...
            axios.post(url + '/promocode', {token: user.token}, {headers: headers})
                .then((res) => {
                    const data = res.data
        ...
        const checkCurrentUserPromo = () => {
            axios
                .post(`https://polarpelmeni-api.work-set.eu/api/auth`, {token: user.token}, {
                    headers: {
                        'Content-Type': 'application/json',
                    },
                })
        ...
        const createTransaction = (amount) => {
            ...
            axios.post(url + '/pay', data, {headers: headers})
                .then((res) => {
                    const data = res.data
        ...
        const checkTransactionStatus = () => {
            ...
            axios.post(url + '/getStatus', data, {headers: headers})
                .then((res) => {
                    const data = res.data
        ...
        const createOrder = () => {
            ...
            axios.post(url + '/createOrder', {order_id: orderId, data}, {headers: headers})
                .then((res) => {
                    const data = res.data
                    if (!data.error) {
        ...
        const onSubmit = () => {
            ...
            if (orderData.phone === '') {
                ...
                if (orderData.payment.type === 1) {
                    createTransaction(isPromotion ? (calculateTotalPrice(shoppingCart) * (60 / 100)) : calculateTotalPrice(shoppingCart))
                }
        ...
        return (
            <>
                {modals.thanksModal && (
                    ...
                    >
                        <Thanks orderId={posterOrder.incoming_order_id}
                                deliveryTime={posterOrder.delivery_time}/>
                    </Popup>
                )}
                {error.status === true && (
                    <PopupActions action={error.currentError} onClick={() => setError({
                        status: false,
                        currentError: '',
                    })} error/>
                )}
                {promotionPopup === true && (
                    <PopupActions action={'Ваш промокод застосован'} onClick={() => {
                        setPromotionPopup(false)
                    }}/>
                )}
